...
public class PixelArtCamera : MonoBehaviour {
	...
	void OnPostRender() {
		...
		/*if (windowboxing) {
			...
			Graphics.Blit(rt, (RenderTexture)null, finalBlitStretch, (finalBlitStretch - Vector2.one) / -2f);
		} else if (useUpscaleShader) {
			upscaleMaterial.SetVector("_DestinationResolution", new Vector4(Screen.width, Screen.height, 0, 0));
			Graphics.Blit(rt, (RenderTexture)null, upscaleMaterial);
		} else {
			...
			Graphics.Blit(largeRt, (RenderTexture)null, finalBlitStretch, (finalBlitStretch - Vector2.one) / -2f);
